...
void setup() {
    ...
    Serial.println(
        "Light / Deep Sleep Test.");  // The serial port prints the formatted
                                      // string with a newline.
                                      // 串口输出格式化字符串并换行
    M5.Lcd.println(
        "Light / Deep Sleep Test.");  // The screen prints the formatted string
                                      // and wraps it.
                                      // 屏幕打印格式化字符串并换行
    ...
    M5.Axp.LightSleep(
        SLEEP_SEC(5));  // Wake up after 5 seconds of light sleep, the CPU will
                        // reboot and the program will start from the beginning.
    ...
    M5.Axp.DeepSleep(
        SLEEP_SEC(5));  // Wake up after 5 seconds of deep sleep, the CPU will
                        // reboot and the program will start from the beginning.
                        // 深度睡眠5秒后唤醒,CPU将重新启动，程序将从头开始执行
}
